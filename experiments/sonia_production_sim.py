#!/usr/bin/env python3
# Production-realistic memory experiment.
#
# Instead of storing every token at every layer (13,000+ cells, 3 hours),
# this stores per-token vectors at ONE semantic layer only.
# 16 memories x ~50 tokens = ~800 cells. Under a minute.
#
# Character: Sonia, 34, freelance translator, Chicago.
# 16 memories across completely different life domains.
#
# Usage:
#     source .venv/bin/activate
#     python experiments/sonia_production_sim.py
...
def run_variant(model, tokenizer, query_layer, mode):
    ...
    for mem_idx, memory in enumerate(MEMORIES):
        ...
        hidden = outputs.hidden_states[query_layer + 1].numpy()[0]

        if mode == "per-token":
            for tok_idx in range(seq_len):
                tok_vec = hidden[tok_idx].astype(np.float32)
                cell_id = engine.mem_write(1, 0, tok_vec, tok_vec, 80.0, None)
                cell_to_mem[cell_id] = mem_idx
        else:
            mean_vec = hidden.mean(axis=0).astype(np.float32)
            cell_id = engine.mem_write(1, 0, mean_vec, mean_vec, 80.0, None)
            cell_to_mem[cell_id] = mem_idx

    print(f"  [{mode}] {engine.cell_count()} cells stored")
...
def main():
    print("=" * 70)
    print("Sonia's Two Weeks -- Multi-Domain Memory Retrieval")
    print("Qwen3-0.6B | Single semantic layer | 16 memories x 16 queries")
    print("=" * 70)
