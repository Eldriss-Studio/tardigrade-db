...
| Python | pytest | 343 | PyO3 bindings, hook ABC, HF KV hook, per-token encoding, KV pack, MCP tools, diagnostics, RAG baseline, vLLM connector/prefix client, synthetic-fact KV injection, prefix builder, retrieval key strategies (17), semantic edges (4), SynapticBank (6), multi-agent (5), connector hardening: metadata bridge (5), fingerprint bounds (3), save-side key unification (5), lifecycle regression (1), thread safety: GIL release (2) + concurrent access (2), `mem_read_tokens` parity (3), refinement API (11), cross-encoder reranker (5), shared constants (13), view generator (18), consolidator (14), consolidation sweep (5), chunker (13), file ingestor (12) |

Per-crate counts include unit + acceptance + doctest tests. Sum: 6+39+60+25+26+144+343 = 643.
