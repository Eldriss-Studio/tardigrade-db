...
|-------|-------|-------|----------------|
| Core | tdb-core | 4 | Builder, SynapticBank, KVPack types, tier defaults, retrieval boost |
| Storage | tdb-storage | 38 | Q4 round-trip, segment rollover, persistence, SynapticStore, TextStore (single + batch), DeletionLog, segment compaction (6) |
| Retrieval | tdb-retrieval | 59 | Per-token Top5Avg, SLB eviction, pipeline, SIMD dot product, owner filter, PerTokenConfig, corpus-mean tracking (3), refinement strategies — None/MeanCentered/LatentPrf (6) |
...
| Engine | tdb-engine | 147 | Write/read, pack API, text storage, delete, state rebuild, SLB chain, Vamana activation, refresh + WAL checkpoint, active governance (tier boost + eviction), semantic edges (Supports/Contradicts), multi-agent isolation (3 agents × 5 packs), status API, crash recovery (segment + WAL truncation), multi-component atomicity (text store, deletion log, flush), `mem_read_tokens` direct token API (3), refinement modes — none/centered/prf (3), `add_view_keys`/`view_count` (7) |
| Python | pytest | 359 | PyO3 bindings, hook ABC, HF KV hook, per-token encoding, KV pack, MCP tools, diagnostics, RAG baseline, vLLM connector/prefix client, synthetic-fact KV injection, prefix builder, retrieval key strategies (17), semantic edges (4), SynapticBank (6), multi-agent (5), connector hardening: metadata bridge (5), fingerprint bounds (3), save-side key unification (5), lifecycle regression (1), thread safety: GIL release (2) + concurrent access (2), `mem_read_tokens` parity (3), refinement API (11), cross-encoder reranker (5), shared constants (13), view generator (18), consolidator v2 — `add_view_keys` + `int` returns (14), consolidation sweep v2 — `views_attached` counter (5), chunker (13), file ingestor (12), client facade v2 — `consolidate()→int` / `consolidate_all()→dict[int,int]` (10) |

Per-crate counts include unit + acceptance + doctest tests. Sum: 4+38+59+24+25+147+359+7(doctests) = 663.
