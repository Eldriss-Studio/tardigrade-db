...
| Python | pytest | 251 | PyO3 bindings, hook ABC, HF KV hook, per-token encoding, KV pack, MCP tools, diagnostics, RAG baseline, vLLM connector/prefix client, synthetic-fact KV injection, prefix builder, retrieval key strategies (17), semantic edges (4), SynapticBank (6), multi-agent (5), connector hardening: metadata bridge (5), fingerprint bounds (3), save-side key unification (5) |

Per-crate counts include unit + acceptance + doctest tests. Sum: 6+33+51+25+26+124+251 = 516.
