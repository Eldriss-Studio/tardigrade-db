...
**Scale validated:** 100% recall at 5K memories (Top5Avg, Q4 pipeline, 100→5K clean scaling). Vamana gives 1.4x latency speedup with zero recall loss. **Thread-safe:** Python Engine wrapped in `Arc<Mutex<>>` with GIL release via `py.detach()`. **Cross-model:** Same-family 90% R@5 via linear projection (Qwen3-0.6B→1.7B); cross-family 76.7% via MLP adapter (Qwen3→GPT-2, ~400K params). **Vague queries:** specific 100% R@5, moderate/vague ~46% R@5 (100 queries per tier). The cliff is vocabulary overlap, not vagueness — retrieval needs augmentation for non-specific queries.
