...
> **🧱 Foundation-completion phase — 2026-05-17.** After the LoCoMo bench audit retracted prior inflated scores (Phase 1B.10), focus moved to making the engine a solid primitive any consumer can plug into. Plan: `~/.claude/plans/i-want-a-definitive-majestic-bear.md` (refreshed). Lessons borrowed from SpacetimeDB's storage and DX patterns (cloned for study at `~/Dev/SpacetimeDB`). Landed milestones:
>
> - **M0 — CLI bootstrap** (`0282db2`). `tardigrade init|store|query|status|consolidate`. Subcommand-per-file (Strategy + Template Method), 2 starter templates (`python-basic`, `rust-basic`), 23 ATs.
> - **M1.1 — Owner registry** (`36680e3`). `Engine::list_owners`, `owner_exists`, `delete_owner` with cascade. 14 ATs (Rust + PyO3 parity).
...
> - **M1.3 — Streaming write buffer** (`99c1482`). `Engine::open_with_write_buffer(dir, BufferConfig { max_batch_size, max_idle_ms })` batches `mem_write_pack` calls behind a single coalesced `pool.append_batch` fsync. Pack ids return synchronously; visibility deferred until `flush_buffer()` or auto-flush at threshold. `Engine::flush()` drains the buffer too. 13 ATs.
> - **M2.1 — HTTP/REST bridge**. `python/tardigrade_http/` exposes `POST /mem/store`, `POST /mem/query`, `GET /mem/owners`, `GET /mem/status`, `POST /mem/save`, `POST /mem/restore` as an **Adapter** over `Engine`. FastAPI app factory (`create_app(engine, kv_fn=None)`) for embedding; env-driven `main()` (binds 127.0.0.1, no auth) for standalone serve. Errors are RFC 7807 `application/problem+json`. Checked-in OpenAPI contract at `python/tardigrade_http/schema.yaml` (regenerated by `python -m tardigrade_http.export_schema`) plus a drift-guard test that fails if the live app diverges from the committed file. 12 ATs.
> - **M2.2 — Consumer guide + Node.js reference**. `docs/guide/consumers.md` documents three integration patterns (turn-by-turn agent, multi-NPC/multi-tenant, document ingest) plus the determinism contract. `examples/nodejs_consumer/index.mjs` is a dependency-free Node 22+ script (`fetch` only) that stores 5 facts, queries them back, snapshots, restores, and asserts owner enumeration. Generated TypeScript type definitions at `python/tardigrade_http/types.ts` (regenerated by `npx openapi-typescript schema.yaml -o types.ts`). 1 AT — uvicorn-in-a-thread spawns the bridge on a free port, runs `node index.mjs` against it, asserts exit 0 + expected stdout line.
>
> Foundation gaps still open: M1.4 honesty follow-up (#118 — make the shared-LLM benchmark comparators real or remove them), M3.x (polish — checkpoints, scheduler, builder pattern, miette diagnostics, consumer demos).
