...
| ~~**LongMemEval 90.9%**~~ **[RETRACTED 2026-05-14]** | Originally cited 500 items / deterministic eval / Qwen3-0.6B + centered. Measured on the corrupted dataset via the lexical fallback adapter, not the native engine. Clean-data full-corpus re-run pending. See [`2026-05-14-bench-audit.md`](2026-05-14-bench-audit.md). | RETRACTED |
| ~~**LoCoMo 68.2%**~~ **[RETRACTED 2026-05-14]** | Originally cited 1,542 items / deterministic eval. Lexical-fallback self-retrieval on a corpus where every item shared one ~62K-char context. Honest native-engine measurement on clean data: ~36% R@1 at 50-item scale; full-corpus pending. See [`2026-05-14-bench-audit.md`](2026-05-14-bench-audit.md). | RETRACTED |
| ~~**LLM-judged benchmark confirms gap is real**~~ **[RETRACTED 2026-05-14]** | DeepSeek LLM judge results (LoCoMo 67.2%, LongMemEval 88.8%) were drawn from the same corrupted dataset and lexical fallback adapter. The "gap is genuine" conclusion is unsupported by these runs. | RETRACTED |
| **ZCA whitening, token reweighting, multi-layer fusion: all 0% improvement** (10-fact synthetic corpus) | 10-fact Sonia corpus results (Specific 100%, Moderate 80%, Vague 60%) are unaffected. ⚠️ **The DeepMind LIMIT / "theoretical ceiling bounded by sign-rank" claim is RETRACTED 2026-05-14** — it was invoked to explain a LoCoMo ceiling that turned out to be the lexical fallback's self-retrieval performance, not a real retrieval ceiling. | Synthetic-corpus number stands; LIMIT-paper inference retracted |
| ~~**Latent-space retrieval ceiling identified**~~ **[RETRACTED 2026-05-14]** | This finding was drawn from the broken LoCoMo runs. There is no empirical evidence for a latent-space retrieval ceiling from these experiments. See [`2026-05-14-bench-audit.md`](2026-05-14-bench-audit.md). | RETRACTED |
| ~~**RLS keyword expansion: 100% on 10-fact, 0% on LoCoMo**~~ The 10-fact result stands. **The LoCoMo "0% improvement" is RETRACTED 2026-05-14** — on clean data, keyword expansion *underperforms* the no-RLS baseline (-5.3pp at 50-item scale). | 10-fact stands; LoCoMo retracted |
| ~~**RLS embedding expansion: 0% on LoCoMo**~~ **[RETRACTED 2026-05-14]** | LoCoMo number measured against the lexical fallback / broken corpus. No honest measurement of embedding-expansion vs native baseline yet. | RETRACTED |
| ~~**LoCoMo gap resists all latent-space-only techniques**~~ **[RETRACTED 2026-05-14]** | Conclusion was drawn from runs against the lexical fallback adapter, not the native engine. Active research path is no longer "RLS to bridge a ceiling" — see audit § Recommendations. | RETRACTED |
...
| **Head-to-head benchmark vs Mem0/Letta/Zep** | TardigradeDB-only baseline needs to be re-established on clean data (the prior 68.2% LoCoMo / 90.9% LongMemEval baselines were retracted on 2026-05-14). Three-way comparison (same corpus, same evaluator) requires Docker stack for Mem0+Letta. | Without side-by-side data on same evaluator, published competitor numbers use different eval methods. |
...
| ~~LoCoMo full benchmark (1,542 items)~~ | **[RETRACTED 2026-05-14]** — measured lexical fallback adapter on corrupted dataset. Honest native-engine: ~36% R@1 at 50-item scale on clean data; full-corpus pending. |
| ~~LongMemEval full benchmark (500 items)~~ | **[RETRACTED 2026-05-14]** — same root cause. Clean-data re-run pending. |
| ~~LoCoMo + LongMemEval with DeepSeek LLM judge~~ | **[RETRACTED 2026-05-14]** — same corrupted dataset; "gap is real" conclusion unsupported. |
| ZCA whitening refinement | Complete (10-fact synthetic) — 0% improvement. LoCoMo invocation retracted. |
| Token importance reweighting | Complete (10-fact synthetic) — 0% improvement. |
| Multi-layer query fusion (RRF) | Complete (10-fact synthetic) — 0% improvement. |
| ~~Stacked whitening + reweight + multi-layer~~ | **[RETRACTED 2026-05-14]** — the "theoretical ceiling confirmed (DeepMind LIMIT)" framing was drawn from broken LoCoMo data. |
| RLS keyword expansion (10-fact) | Complete — 100% all tiers (+40pp vague). Hand-crafted synonyms. |
| ~~RLS keyword expansion (LoCoMo)~~ | **[RETRACTED 2026-05-14]** — clean-data measurement shows -5.3pp vs no-RLS baseline. |
| ~~RLS embedding expansion (LoCoMo)~~ | **[RETRACTED 2026-05-14]** — measured against broken baseline. |
| ~~RLS generative 3B (LoCoMo)~~ | **[RETRACTED 2026-05-14]** — same root cause. The "68.2% ceiling" claim is not empirically supported. |
| ~~Chunked ingestion (LoCoMo)~~ | **[RETRACTED 2026-05-14]** — chunked ingestion as a code change is still implemented and tested; the LoCoMo result was measured against the broken baseline. |
| ~~LLM agent reformulation — naive fusion (LoCoMo)~~ | **[RETRACTED 2026-05-14]** — clean-data result: agent reformulator loses 12.7pp vs no-RLS baseline (none 21.95% → agent 9.29% at 50 items). The "-15.3pp" figure was relative to a broken baseline. |
| ~~LLM agent reformulation — naive fusion (LongMemEval)~~ | **[RETRACTED 2026-05-14]** — same root cause. |
| **RLS deprioritized** | All RLS modes underperform no-RLS baseline on clean data per 2026-05-14 audit. Margin-based acceptance / native-CUDA baseline work were follow-ups to a retracted ceiling claim — no longer the active path. See [`2026-05-14-bench-audit.md`](2026-05-14-bench-audit.md) § Recommendations going forward. |
