...
**Reranker conclusion from this body of work.** The cross-encoder pattern is correct (validated by ZeroEntropy controlled comparison and MemReranker's beat of GPT-4o-mini at <1% cost). Our specific cross-encoder model is wrong for LoCoMo dialogue. **Recommended next default:** `IAAR-Shanghai/MemReranker-0.6B` for measured dialogue + memory performance, fitting our VRAM constraints, Apache 2.0 license. **A/B before locking in** — the paper does not benchmark MS-MARCO-MiniLM directly, so the LoCoMo lift is inferred (large) rather than measured.
