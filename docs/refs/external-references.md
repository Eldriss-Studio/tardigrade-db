...
### A3h. Cross-Encoder Rerankers for Dialogue Memory (2026-05-15 audit)

Commissioned after Phase 1B of the retrieval audit (`docs/experiments/2026-05-14-bench-audit.md`) found the cross-encoder reranker was being passed the parent item's full context instead of per-chunk text. The fix lifted LongMemEval from 3.14% → 93.6% on a 50-item subset, but LoCoMo capped at 38% because the production reranker (`cross-encoder/ms-marco-MiniLM-L-6-v2`) systematically picks conversationally-affirming text over factually-relevant text on dialogue queries. This section catalogs the modern reranker landscape and pins MemReranker-0.6B as the recommended next default.

| Paper / Source | arXiv / Link | What it justifies | Where cited |
|---|---|---|---|
| **MemReranker** (Reasoning-Aware Reranking for Agent Memory Retrieval; IAAR-Shanghai, May 2026) | [arXiv:2605.06132](https://arxiv.org/abs/2605.06132) | Purpose-built dialogue reranker. Qwen3-Reranker-0.6B distilled with ~1M general + ~50K multi-turn dialogue pairs; "answer density and signal-to-noise ratio (15%)" label dimension explicitly targets the "conversationally-affirming wins over content-relevant" failure mode. Reports **LoCoMo 0.7150 MAP / LongMemEval 0.7538 MAP** vs `bge-reranker-v2-m3`'s 0.6708 / 0.7069. On LongMemEval beats GPT-4o-mini-as-reranker (0.5684); on LoCoMo *ties* with it (0.7150 vs 0.7151). Apache 2.0 weights. **NOT openly available — see availability note below.** | reranker research (2026-05-15) |
| **MemReranker-0.6B model card** | [huggingface.co/IAAR-Shanghai/MemReranker-0.6B](https://huggingface.co/IAAR-Shanghai/MemReranker-0.6B) | **PRIVATE — HTTP 401.** Verified 2026-05-15: the IAAR-Shanghai org's public model listing contains MemReranker-4B but no 0.6B entry. The 4B model card includes a note "No GPU? Use our hosted API" pointing at the Memos Rerank API; the 0.6B is intentionally an API-only commercial SKU, not pulled, not license-gated. To access weights: contact IAAR-Shanghai or use their commercial API ([memos-docs.openmem.net](https://memos-docs.openmem.net/cn/api_docs/core/rerank)). Self-train workaround: paper's recipe + Qwen3-Reranker-0.6B base (training data partly proprietary, partial reproduction feasible) | reranker research + availability investigation (2026-05-15) |
| **Qwen3-Reranker-0.6B as the open replacement** | [huggingface.co/Qwen/Qwen3-Reranker-0.6B](https://huggingface.co/Qwen/Qwen3-Reranker-0.6B) | The base model from which MemReranker was distilled. Per MemReranker paper Table 5: **0.6427 LoCoMo MAP / 0.5945 LongMemEval MAP** (below MemReranker but above current production MS-MARCO-MiniLM). Apache 2.0, ~1.2 GB VRAM at BF16, instruction-aware. **The only model in our 2026-05-15 case-study shootout** to correctly pick the right chunk for "When did Caroline apply to adoption agencies?" — beat MiniLM, BGE-v2-m3, mxbai-v2 by a 4-point margin. **New recommended default reranker** after MemReranker was found inaccessible | reranker research (2026-05-15); committed as default in `_RERANK_MODEL` env override planned |
| **MemReranker-4B model card** | [huggingface.co/IAAR-Shanghai/MemReranker-4B](https://huggingface.co/IAAR-Shanghai/MemReranker-4B) | Larger variant: LoCoMo 0.737 / LongMemEval 0.8043 MAP. ~8 GB VRAM — too large for the 8 GB 3070 Ti alongside Qwen3-0.6B, but the architectural reference | reranker research (2026-05-15) |
| **Qwen3-Reranker-0.6B** | [huggingface.co/Qwen/Qwen3-Reranker-0.6B](https://huggingface.co/Qwen/Qwen3-Reranker-0.6B) | Base of MemReranker. Causal-LM "yes/no" head, instruction-aware (1-5% lift with task hints), 32K context. Strong MTEB-R baseline. Apache 2.0 | reranker research (2026-05-15) |
| **Qwen3-Embedding technical report** | [arXiv:2506.05176](https://arxiv.org/pdf/2506.05176) | Qwen3 embedding/reranker family architecture, training recipe, MTEB results. Backbone for MemReranker | reranker research (2026-05-15) |
| **BAAI/bge-reranker-v2-m3** | [huggingface.co/BAAI/bge-reranker-v2-m3](https://huggingface.co/BAAI/bge-reranker-v2-m3) | XLM-RoBERTa cross-encoder, 568M params, ~1.2 GB BF16. Apache 2.0. Strong on BEIR/MIRACL; the head-to-head baseline MemReranker beats on LoCoMo/LongMemEval | reranker research (2026-05-15) |
| **mxbai-rerank-v2** (Mixedbread, 2025) | [mixedbread.com/blog/mxbai-rerank-v2](https://www.mixedbread.com/blog/mxbai-rerank-v2) | Qwen-2.5 base + GRPO+contrastive training. `base-v2` (0.5B) 55.57 BEIR NDCG@10, `large-v2` (1.5B) 57.49. Apache 2.0. Best general-purpose 2025 reranker; **recommended fallback** if MemReranker disappoints in A/B | reranker research (2026-05-15) |
| **Should You Use LLMs for Reranking?** (ZeroEntropy, 2025) | [zeroentropy.dev](https://www.zeroentropy.dev/articles/should-you-use-llms-for-reranking-a-deep-dive-into-pointwise-listwise-and-cross-encoders) | Controlled comparison: cross-encoder `zerank-1` 0.777 NDCG@10 vs GPT-5-mini ~0.70. CE wins on accuracy AND latency AND cost. Pointwise LLM scoring is uncalibrated; listwise LLM ranking is fragile beyond ~10 candidates. Verdict: "Cross-encoders should be your go-to tool for reranking." Counter-evidence to "drop the reranker, use LLM" | reranker research (2026-05-15) |
| **Cross-encoder length sensitivity** (Bedi et al., 2022) | [arXiv:2207.01262](https://arxiv.org/html/2207.01262v2/) | Cross-encoders drop ~9 NDCG@10 when documents shrink from 214 → 79 tokens. LoCoMo evidence is 30-300 chars (~10-75 tokens) — squarely in the regime where MS-MARCO-trained CEs degrade. Pins the MiniLM-on-short-text failure | reranker research (2026-05-15) |
| **Agentmemory on LongMemEval** (omegamax.co, 2025) | [omegamax.co/blog/number-one-on-longmemeval](https://omegamax.co/blog/number-one-on-longmemeval) | Top LongMemEval system at 96.2% uses `cross-encoder/ms-marco-MiniLM-L-6-v2` — so MiniLM is not strictly fatal at the system level with heavy retrieval-fusion compensation. Confirms our 93.6% LongMemEval result with MiniLM is achievable | reranker research (2026-05-15) |
| **Hindsight LongMemEval writeup** | [dev.to/jordanmccann](https://dev.to/jordanmccann/how-i-built-a-memory-system-that-scores-962-on-longmemeval-1-in-the-world-41n) | 96.2% LongMemEval blog. Architecture: BM25+vector+RRF+temporal boosting; same MiniLM reranker. Confirms our finding that the reranker is the bottleneck, not the LongMemEval-direction signal | reranker research (2026-05-15) |
| **Emergence LongMemEval SOTA writeup** | [emergence.ai/blog/sota-on-longmemeval-with-rag](https://www.emergence.ai/blog/sota-on-longmemeval-with-rag) | Cohort analysis of LongMemEval-winning systems. Reranker stage universally present in top systems | reranker research (2026-05-15) |
| **ConvoMem benchmark** (Liang et al., 2025) | [arXiv:2511.10523](https://arxiv.org/pdf/2511.10523) | Conversational memory benchmark. Finding: for <150-conversation histories, naïve full-context QA beats RAG+rerank — interesting scale-dependence point but doesn't apply to our LongMemEval-115K-token scale | reranker research (2026-05-15) |
| **Reranker benchmark survey** (Liang et al., 2024) | [arXiv:2409.07691](https://arxiv.org/pdf/2409.07691) | Benchmarking, fine-tuning, deploying rerankers for RAG. Methodology baseline for our A/B framework | reranker research (2026-05-15) |
| **Sentence-Transformers — Pretrained CE models** | [sbert.net/docs/cross_encoder/pretrained_models.html](https://sbert.net/docs/cross_encoder/pretrained_models.html) | Official SBERT documentation explicitly flags MS-MARCO-MiniLM as "older, smaller" and outclassed by newer models. Authoritative source for the recommendation to swap | reranker research (2026-05-15) |
| **Sentence-Transformers issue #2874** | [github.com/UKPLab/sentence-transformers/issues/2874](https://github.com/UKPLab/sentence-transformers/issues/2874) | Community report of low CE scores on short texts (LoCoMo regime). Confirms length-sensitivity issue | reranker research (2026-05-15) |
| **MemMachine v0.2 on LoCoMo** | [memmachine.ai/blog/2025/12/...](https://memmachine.ai/blog/2025/12/memmachine-v0.2-delivers-top-scores-and-efficiency-on-locomo-benchmark/) | Field comparison data point — does not isolate reranker contribution | reranker research (2026-05-15) |

**Reranker conclusion from this body of work.** The cross-encoder pattern is correct (validated by ZeroEntropy controlled comparison; MemReranker beats GPT-4o-mini on LongMemEval but only *ties* on LoCoMo per arXiv:2605.06132 Table 5, contrary to the earlier claim). **Recommended default reranker:** `Qwen/Qwen3-Reranker-0.6B` — Apache 2.0, fits VRAM, was the only model in our 2026-05-15 case-study shootout to correctly rank a temporal dialogue query above conversationally-affirming distractors. MemReranker-0.6B would be marginally better per its paper but is HTTP 401 / API-only.

### A3i. LoCoMo Evaluation Protocol — Retrieve-Then-Read (2026-05-15 audit)

Investigation prompted by the discovery that 27.6% of LoCoMo items share their context with another item (`docs/experiments/2026-05-14-bench-audit.md` Phase 1B.4). TardigradeDB's bench adapter does `answer = mapped.ground_truth` which collapses N-way duplicates to 1, capping deterministic scoring at 20-50% per duplicate cluster regardless of retrieval quality. This section catalogs the standard fix and confirms the duplicate-context artifact is solved by the standard retrieve-then-read pipeline.

| Paper / Source | arXiv / Link | What it justifies | Where cited |
|---|---|---|---|
| **LoCoMo paper** (Maharana et al., ACL 2024) | [arXiv:2402.17753](https://arxiv.org/abs/2402.17753), [project page](https://snap-research.github.io/locomo/) | Official protocol: retrieve → LLM-generate short answer → F1/BLEU + LLM-as-Judge. Paper itself contains the load-bearing example (GT "Alice was born in March" vs generated "Alice is born in July" — both score high on F1/BLEU despite being semantically opposite). LLM judge is mandated to handle this. | LoCoMo evaluation research (2026-05-15) |
| **LoCoMo official repo + eval scripts** | [github.com/snap-research/locomo](https://github.com/snap-research/locomo) | `evaluate_gpts.sh`, `evaluate_claude.sh`, `evaluate_gemini.sh`, `evaluate_rag_gpts.sh` all use generator+judge LLMs. README documents the retrieval-augmented variant explicitly. Authoritative source for "the official protocol is retrieve-then-read, not stored-GT-comparison" | LoCoMo evaluation research (2026-05-15) |
| **LoCoMo issue #38 — Engram zero-LLM retrieval** | [github.com/snap-research/locomo/issues/38](https://github.com/snap-research/locomo/issues/38) | Engram reports R@5 = 93.9% with zero-LLM retrieval, explicitly distinguishing it from leaderboard LLM-Judge numbers. **Validates the option of publishing R@k as a retrieval diagnostic distinct from a LoCoMo Judge claim** — exactly the framing TardigradeDB needs until the LLM-gated path is wired | LoCoMo evaluation research (2026-05-15) |
| **Mem0 evaluation code** | [github.com/mem0ai/mem0/tree/main/evaluation](https://github.com/mem0ai/mem0/tree/main/evaluation) | Defaults: `gpt-4o-mini` as eval-LLM (generator) AND `gpt-4o-mini` as judge-LLM. Mem0's headline "26% relative improvement over OpenAI Memory" is on the LLM-Judge metric, not F1. Confirms every leaderboard system uses retrieve-then-LLM-generate-then-LLM-judge | LoCoMo evaluation research (2026-05-15) |
| **Memobase LoCoMo bench README** | [github.com/memodb-io/memobase/.../locomo-benchmark/README.md](https://github.com/memodb-io/memobase/blob/main/docs/experiments/locomo-benchmark/README.md) | "LLM Judge Score uses LLM (e.g. OpenAI gpt-4o) to judge if the answer generated from memory method is the same as the ground truth, score is 1 if it is, else 0." Reports 75.78% LLM-Judge headline. Same pattern as Mem0 | LoCoMo evaluation research (2026-05-15) |
| **ByteRover 2.0 LoCoMo writeup — generator + judge** | [byterover.dev/blog/benchmark-ai-agent-memory](https://www.byterover.dev/blog/benchmark-ai-agent-memory) | Three-stage Retrieve → Justify (Gemini 3 generates answer) → Judge. Their 92.2% is an LLM-Judge over LLM-generated answers, not retrieval@k. Same pattern again | LoCoMo evaluation research (2026-05-15) |
| **MemMachine v0.2 LoCoMo writeup** | [memmachine.ai/blog/2025/12/memmachine-v0.2-delivers-top-scores-and-efficiency-on-locomo-benchmark/](https://memmachine.ai/blog/2025/12/memmachine-v0.2-delivers-top-scores-and-efficiency-on-locomo-benchmark/) | GPT-4.1 judge, temperature 0.1, "2-3% variance across runs" — even with deterministic decoding the LLM-Judge metric has measurable variance | LoCoMo evaluation research (2026-05-15) |
| **dial481/locomo-audit — independent audit** | [github.com/dial481/locomo-audit](https://github.com/dial481/locomo-audit) | 99/1540 (6.4%) golden answers wrong; LLM judge accepts 62.81% of intentionally-wrong-but-vague-but-topical answers; third-party reproductions of EverMemOS hit 38.38% vs claimed 92.32%; 446 adversarial questions (22.5%) silently skipped by published eval code. **The LoCoMo leaderboard is methodologically noisy** in directions aligning with our audit's stance | LoCoMo evaluation research (2026-05-15) |
| **Zep retraction** | [github.com/getzep/zep-papers/issues/5](https://github.com/getzep/zep-papers/issues/5) | Zep retracted their 84% LoCoMo claim to 58.44% after fixing their own eval. Another data point on leaderboard noise; supports cautious-comparison framing | LoCoMo evaluation research (2026-05-15) |
| **DPR — Dense Passage Retrieval** (Karpukhin et al., 2020) | [arXiv:2004.04906](https://arxiv.org/abs/2004.04906) | Canonical retrieve-then-read pipeline for open-domain QA. Retrieval (DPR) and reader (BERT extractive) are separate; evaluation is Exact Match + F1 on the reader's output. Demonstrates the "score retrieval as Recall@k, score QA as answer-string F1" separation that LoCoMo systems follow | LoCoMo evaluation research (2026-05-15) |
| **RAG — Retrieval-Augmented Generation** (Lewis et al., 2020) | [arXiv:2005.11401](https://arxiv.org/abs/2005.11401) | Generative variant of retrieve-then-read. Architectural reference for the LLM-gated bench mode TardigradeDB needs to wire | LoCoMo evaluation research (2026-05-15) |
| **DrQA — Reading Wikipedia to Answer Open-Domain Questions** (Chen et al., 2017) | [arXiv:1704.00051](https://arxiv.org/abs/1704.00051) | Established the retriever + reader pattern that DPR/RAG inherit | LoCoMo evaluation research (2026-05-15) |
| **CoQA — Conversational QA** (Reddy et al., 2019) | [arXiv:1808.07042](https://arxiv.org/abs/1808.07042) | Single-passage, many-questions-per-passage benchmark (by design). Per-question F1 against the answer span, computed by an extractive or generative reader given (passage, dialogue history, current question). Confirms "passage is shared across questions" is a *standard* IR/QA property, handled by always running the reader. The LoCoMo duplicate-context artifact is a known pattern, not a bug | LoCoMo evaluation research (2026-05-15) |
| **LongMemEval has no duplicate-context items** | [arXiv:2410.10813 HTML](https://arxiv.org/html/2410.10813v2) | "Each of the 500 questions has a unique, non-shared chat history." TardigradeDB's 93%+ LongMemEval result is not vulnerable to the duplicate-context artifact that affects LoCoMo | LoCoMo evaluation research (2026-05-15) |

**Evaluation conclusion from this body of work.** The LoCoMo duplicate-context artifact has a standard fix (retrieve-then-LLM-generate-then-LLM-judge) used by every leaderboard system and validated by the IR/QA literature (DPR, RAG, DrQA, CoQA, BEIR). TardigradeDB's `answer = mapped.ground_truth` pattern is non-standard; until the LLM-gated path is wired, bench scores should be published as **retrieval diagnostics (R@k)**, not as LoCoMo Judge scores. The bench harness already supports `llm_gated` evaluator mode; the adapter needs to feed retrieved evidence + question to the answerer LLM rather than returning stored ground-truths verbatim.
...
| **ZCA whitening, token reweighting, multi-layer fusion** (latent-space geometric transforms) | All three tested 2026-05-11 on 10-fact Sonia corpus: 0% improvement on any vague query in any configuration or combination (synthetic-corpus result stands). ⚠️ The "DeepMind LIMIT proves the vocabulary-mismatch ceiling on TardigradeDB" extrapolation is **RETRACTED 2026-05-14** — that inference was supported only by the broken LoCoMo runs. Architecture preserved (RefinementStrategy trait, TokenWeighter, MultiLayerQuery) but does not improve recall on the 10-fact synthetic corpus |
| **RLS embedding expansion** (nearest-neighbor vocabulary bridge from model's embedding table) | ~~Tested 2026-05-11 on full LoCoMo (1,542 items): 67.2%→67.2% (0% improvement).~~ ⚠️ **[RETRACTED 2026-05-14]** — the 67.2% baseline was the lexical fallback adapter on a corrupted dataset; the "0% improvement" delta is meaningless. Clean-data 50-item baseline (no-RLS) is ~21.95% R@1; clean-data measurement of embedding expansion is not yet available. Architecture (EmbeddingExpansionStrategy) is intact and can be re-evaluated on the clean dataset |
